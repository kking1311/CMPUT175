...
def dequeue_experiment(queue):
    ...
    while not queue.isEmpty():  # Dequeue until the queue is empty
        try:
            queue.dequeue()
        except Exception as e:
            print("Error:", e)  # Handle exceptions if any during dequeue operation
    end_time = time.time()  # Record the end time
    time_interval = end_time - start_time  # Calculate the time taken for dequeue operation
